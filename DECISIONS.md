...
## 14. Export：产品文档 §7.7 写了 Export Agent，实际决定不做成 Agent

**最初设想（产品文档 §7.7）**：7 个 Agent 之一，"负责将最终结果导出成不同格式（PDF/Markdown/TXT/简单 TAB）"。

**实际决策**：不包一层 Agent，就是普通的后端函数——输入 `Arrangement`，按模板拼出 Markdown 字符串 / 用 PDF 库渲染，纯格式转换。

**为什么**：回到 [DECISIONS.md](DECISIONS.md) #6 定下的架构原则——"规则系统负责算事实（确定性、可复现），Agent 负责在事实基础上做决策 + 用自然语言解释"。导出这一步输入输出完全确定，没有任何环节需要 LLM 判断、选择或生成解释文字，不满足"需要 Agent"的判断标准。这条早在 PROGRESS.md 里程碑 5 的架构说明里就写过一句"Export 大概率不需要 LLM 决策（纯格式转换）"，这次做的时候正式确认并执行。

**这说明的方法论**：产品文档里定义的 7 个 Agent 划分，是"职责粒度划分"层面的设计，不代表每一个都必须真的实现成一个独立的 LLM Agent——工程实现阶段要针对每个模块单独判断"这里到底需不需要 LLM 参与"，而不是照单全收。Audio Analysis Agent 当初也是同样判断（归入里程碑 4 的纯信号处理 `services/audio/`，没有单独包一层 LLM），Export 是第二次做同类判断。

**面试话术**：这条适合讲"怎么判断一个模块是否真的需要 Agent/LLM"——不是看产品文档写没写"Agent"这个词，而是看这个模块的输入输出之间是否存在需要推理、判断、生成自然语言的环节。导出功能显然没有，所以即使文档称它为 Agent，工程实现上也不应该为了"凑够 7 个 Agent"而强行包一层 LLM 调用，那是不必要的复杂度和成本。
