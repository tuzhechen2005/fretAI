...
### 部署与生产化（2026-07-29 补充：用户问"如果要上线该怎么做"，明确要的是"真正扛得住并发流量"这一档，不是"能演示的最小可行部署"；先记录范围，暂不动手，等前面几个里程碑做完再回来系统性做）
- [ ] **托管平台选型**：前端（Vercel 之类）与后端（容器平台/云主机）分离部署的选型对比，记录为什么选某个平台（成本、国内访问速度、是否需要备案等真实约束）
- [ ] **数据库迁移到 PostgreSQL 真正落地**：呼应 DECISIONS.md #1 早就写明的"如果上生产需要迁移"，这次要真正做一次迁移（不只是理论上"因为用了 ORM 所以可逆"），包括数据搬迁脚本、JSON 列改 JSONB 的验证
- [ ] **密钥与配置管理**：`.env` 里的 LLM API Key 等敏感信息如何在生产环境安全管理（平台的 secret manager，而不是明文环境变量或提交进仓库）
- [ ] **音频文件存储迁移到对象存储**：现在存本地 `storage/uploads/`，生产环境（尤其是无状态容器/多实例场景）文件会丢失或不同实例间不共享，需要迁移到 OSS/S3 类服务
- [ ] **任务队列生产化**：呼应 DECISIONS.md #3，`BackgroundTasks` 没有持久化，服务重启会丢失正在跑的分析任务，生产环境需要 Celery+Redis 或等价方案保证任务可靠性
- [ ] **成本与访问控制**：公开可访问的链接意味着任何人触发的操作都会产生真实 LLM API 费用，需要设计限流/邀请码/用量上限等机制，避免被刷
- [ ] **真实压测数据**：不只是理论分析，要跑出"当前架构在多少并发下开始出问题"的具体数字（呼应下面"延迟与并发"板块的压力测试项，压测应该在真实部署环境下跑，不是本地空转）
- [ ] **HTTPS/域名/CORS 生产配置**：本地开发的 `CORS_ORIGINS=http://localhost:3000` 之类配置需要按生产域名重新设计，不能图省事直接放开所有来源
